...
public class SubReactor implements Runnable {
    ...
    private void register() {
        try {
            ...
            if (StringUtils.equals(ChannelEventModelEnum.TEXT_TRANSMISSION.getName(), subReactor)) {
                ...
                socketChannelContext.getChannelPipeLine().addHandler(new SimpleChannelContext(),
                        new ChatDecodeHandler());
                socketChannelContext.getChannelPipeLine().addHandler(new SimpleChannelContext(),
                        new ChatRealDataHandler());
            ...
            if (eventOpt == 0 || null == socketChannelContext) {
                throw new RuntimeException(
                        "subReactor Thread register failed, can not assign event opt or create attachment for socketChannel.....");
            }

            // 3、通道注册添加附件
            NioServerContext.EventRegister(socketChannel, this.selector, eventOpt).attach(socketChannelContext);
            log.info("[ " + LocalTime.formatDate(LocalDateTime.now()) + " ] SubReactor | --> 聊天服务通道 [{}] 注册成功",
                    socketChannelContext.getRemoteAddress());
        } catch (Exception e) {
            log.error(
                    "[ " + LocalTime.formatDate(LocalDateTime.now())
                            + " ] SubReactor | --> 聊天服务通道 [{}] 注册失败, error = {}",
                    socketChannelContext.getRemoteAddress(), e.getMessage());
    ...
    private SocketChannelContext createModel(SocketChannel socketChannel) throws IOException {
        ...
        this.socketChannel.socket()
                .setSoTimeout(Integer.parseInt(NioServerContext.getValue(BasicConstant.SOCKET_TIMEOUT)));
    ...
    public void run() {
        ...
        log.info(
                "[ " + LocalTime.formatDate(LocalDateTime.now())
                        + " ] SubReactor | --> subReactor thread shutdown success, address = {}, thread = {}",
                socketChannelContext.getRemoteAddress(), Thread.currentThread().getName());
    }
