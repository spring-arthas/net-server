...
public class AbstractSelector {
     ...
     * @param selectionKey
     */
    public void setEventModel(ChannelEventModel channelEventModel, SelectionKey selectionKey) {
        channelEventModel.setSelectionKey(selectionKey);

        // 客户端通道(包括上传或下载客户端通道)
        if (selectionKey.channel() instanceof SocketChannel) {
            SocketChannelContext socketChannelContext = (SocketChannelContext) selectionKey.attachment();
            // 设置可选键对应的远端地址
            channelEventModel.setRemoteAddress(socketChannelContext.getRemoteAddress());
            ...
            if (socketChannelContext.getLocalAddress().contains(NioServerContext.getValue(BasicConstant.NIO_FILE_UPLOAD_PORT))) {
                channelEventModel.setEventModelEnum(ChannelEventModelEnum.FILE_UPLOAD);
            }
            // 如果可选键对应的本地地址为文件上传端口，则记录问文件上传数据
        ...
        if (selectionKey.channel() instanceof ServerSocketChannel) {
            // 设置本地地址
            channelEventModel.setLocalAddress(
                    NioServerContext.getServerLocalAddress(((ServerSocketChannel) selectionKey.channel())));
            // 根据本地对外暴露端口号判断是文件上传socket还是下载socket，以此来设置eventModel当前事件类型
            if (channelEventModel.getLocalAddress().contains(NioServerContext.getValue(BasicConstant.NIO_FILE_UPLOAD_PORT))) {
                channelEventModel.setEventModelEnum(ChannelEventModelEnum.FILE_UPLOAD);
            }
