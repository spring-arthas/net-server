 ...
 *
 *  断点续传文件发送或是上传: 发起者向10087端口建立连接开始进行文件的操作，首先是对文件进行验证
 *
 */

@Slf4j
@SuppressWarnings("all")
public class MainFileResumeSelector extends AbstractSelector implements Runnable {

    @Override
    public void run() {
        Selector selector = super.getCheck(BasicConstant.NIO_SERVER_MAIN_CORE_FILE_RESUME_SELECTOR);
        ...
        while (true) {
            try {
                ...
                log.error("MainFileResumeSelector事件处理：多路复用处理错误, error = {}", ExceptionUtils.getStackTrace(e));
            }
        }
    }
}
