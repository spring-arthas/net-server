...
public class FileTransportManager {
     ...
     * @return 文件在线传输结果
     */
    public static void realOnlineFileDataHandler(SocketChannelContext socketChannelContext,
            FileMessageFrame fileMessageFrame, ChannelContext channelContext) {
        // 1、查询发起端和接收端用户
        ...
        if (!fileTransport.getIsConfirmReceive()) {
            ...
            SubReactor subReactor = GlobalMainReactor
                    .getSubReactorForSocketChannel(BasicServer.classPathXmlApplicationContext.getBean(UserService.class)
                            .getOnlineUser(userQueryParam).getFileSocketChannel());
            sendMessage(fileMessageFrame.getFileTransport(), subReactor.getSocketChannelContext(),
                    FileMessageFrame.FrameType.ONLINE_TRANSPORT.getBit(), FileOperateEnum.ONLINE_TRANSPORT_CANCEL);
        }

        userQueryParam.setUserName(fileMessageFrame.getFileTransport().getLaunchUserName());
        UserDTO launchUserDto = BasicServer.classPathXmlApplicationContext.getBean(UserService.class)
                .getOnlineUser(userQueryParam);
        userQueryParam.setUserName(fileMessageFrame.getFileTransport().getReceiveUserName());
        UserDTO receiveUserDto = BasicServer.classPathXmlApplicationContext.getBean(UserService.class)
                .getOnlineUser(userQueryParam);
        receiveUserDto.setFileSocketChannel(socketChannelContext.getSocketChannel());

        // 3、向发起端用户发送开始传送文件流的通知
        sendMessage(fileTransport,
                GlobalMainReactor.getSubReactorForSocketChannel(launchUserDto.getFileSocketChannel())
                        .getSocketChannelContext(),
                FileMessageFrame.FrameType.ONLINE_TRANSPORT.getBit(), FileOperateEnum.ONLINE_TRANSPORT_CONFIRM);
    }
     ...
     */
    public static void realUploadFileDataHandler(SocketChannelContext socketChannelContext,
            FileMessageFrame fileMessageFrame, ChannelContext channelContext) {
        // 1、解析文件在线传输帧
        FileTransport fileTransport = JSON.parseObject(fileMessageFrame.getData(), FileTransport.class);
        fileMessageFrame.setFileTransport(fileTransport);
        ...
        if (fileMessageFrame.getFileOperateType().equals(FileMessageFrame.FileOperateType.TRANSPORT)) {
            // 2.1、获取用户服务类
            UserQueryParam userQueryParam = new UserQueryParam();
            UserService userService = (UserService) BasicServer.classPathXmlApplicationContext
            ...
            launchUserDto.setFileSocketChannel(socketChannelContext.getSocketChannel());
            ...
            map.put("message", "待处理通知: 您有一条来自用户 [ " + launchUserDto.getUserName() + " ] 传送的文件 [ "
                    + fileMessageFrame.getFileTransport().getFileName() + " ]");
            ...
            log.info("[ " + LocalTime.formatDate(LocalDateTime.now())
                    + " ] FileTransportManager | --> 接收用户 = [{}] 接收到一条来自用户 = [{}] 发送的在线文件实时传输, 需要接收端用户确认是否接收, thread = {}",
                    receiveUserDto.getUserName(), launchUserDto.getUserName(), Thread.currentThread().getName());
            WriteEventHandler.addSendData(map, GlobalMainReactor
                    .getSubReactorForSocketChannel(receiveUserDto.getChatSocketChannel()).getSocketChannelContext());
        }
     ...
     */
    private static void fileStoreHandler(SocketChannelContext socketChannelContext, FileMessageFrame fileMessageFrame,
            String basePath) {
        ...
        if (executeFile.exists()) {
            log.info("[ " + LocalTime.formatDate(LocalDateTime.now())
                    + " ] FileTransportManager | --> 待上传文件 = [{}] 校验完成，当前目录下 = [{}] 已存在, 请修改文件名称后上传, thread = {}",
            ...
            map.put("message", "[ " + fileMessageFrame.getFileTransport().getFileName() + " ] 校验完成, 当前目录下 ["
                    + fileTransport.getGroup() + "] 已存在, 请修改文件名称后上传");
        ...
        try {
            ...
            if (!fileMap.containsKey(fileTransport.getTag())) {
                ...
                map.put("FILE_CHANNEL",
                        FileChannel.open(executeFile.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE));
                fileMap.put(fileTransport.getTag(), map);

                log.info(
                        "[ " + LocalTime.formatDate(LocalDateTime.now())
                                + " ] FileTransportManager | --> 文件 [{}] 上传通道创建成功, 上传路径 = [{}], thread = {}",
                        fileTransport.getFileName(), executeFile.toPath().toAbsolutePath(),
            ...
            log.info(
                    "[ " + LocalTime.formatDate(LocalDateTime.now())
                            + " ] FileTransportManager | --> 待上传文件 = [{}] 校验完成，允许客户端 = [{}] 发送文件数据, thread = {}",
                    fileTransport.getFileName(), fileTransport.getLaunchUserName(), Thread.currentThread().getName());
            WriteEventHandler.addSendData(map, socketChannelContext);
        } catch (IOException e) {
            log.error(
                    "[" + LocalTime.formatDate(LocalDateTime.now())
                            + "] FileTransportManager | --> 文件 [{}] 上传帧处理 error, path = {}, error = {}",
                    fileTransport.getFileName(), executeFile.toPath().toAbsolutePath(), e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     * @return
     */
    public static Boolean realDownloadFileDataHandler(SocketChannelContext socketChannelContext,
            FileMessageFrame fileMessageFrame, ChannelContext channelContext) {
        // 1、解析文件在线传输帧
        FileTransport fileTransport = JSON.parseObject(fileMessageFrame.getData(), FileTransport.class);
        fileMessageFrame.setFileTransport(fileTransport);
        ...
        if (fileDto == null) {
            ...
            log.info(
                    "[ " + LocalTime.formatDate(LocalDateTime.now())
                            + " ] FileTransportManager | --> 待下载文件 = [{}] 校验完成，DB不存在文件记录, thread = {}",
            ...
            WriteEventHandler.addSendData(createDownloadFileErrorMap(message,
                    FileOperateEnum.DOWNLOAD_TRANSPORT_CONFIRM_NOT_EXIST.getOperate()), socketChannelContext);
            return Boolean.FALSE;
        }
        ...
        if (!executeFile.exists()) {
            ...
            log.info(
                    "[ " + LocalTime.formatDate(LocalDateTime.now())
                            + " ] FileTransportManager | --> 待上传文件 = [{}] 校验完成，文件系统中不存在, thread = {}",
                    fileTransport.getFileName(), Thread.currentThread().getName());
        ...
        try {
            ...
            FileWriteEventParseUtil.parseMessageAndSend(socketChannelContext.getSocketChannel(),
                    sendMap, socketChannelContext);
            log.info(
                    "[ " + LocalTime.formatDate(LocalDateTime.now())
                            + " ] FileTransportManager | --> 待下载文件 = [{}] 校验完成，允许客户端 = [{}] 发送文件数据, thread = {}",
                    fileTransport.getFileName(), fileTransport.getLaunchUserName(), Thread.currentThread().getName());
        } catch (IOException e) {
            log.error(
                    "[" + LocalTime.formatDate(LocalDateTime.now())
                            + "] FileTransportManager | --> 文件下载帧处理 error, path = {}, error = {}",
                    executeFile.toPath().toAbsolutePath(), e.getMessage());
            e.printStackTrace();
            return Boolean.FALSE;
        }
     ...
     */
    private static void sendMessage(FileTransport fileTransport, SocketChannelContext socketChannelContext, String bit,
            FileOperateEnum fileOperateEnum) {
        Map<String, Object> map = new HashMap<>(8);
        map.put("code", 200);
        ...
        log.info("[ " + LocalTime.formatDate(LocalDateTime.now())
                + " ] FileOnlineTransportSpecificHandler | --> file online transport send begin receive message success, 发起用户 = {}, thread = {}",
