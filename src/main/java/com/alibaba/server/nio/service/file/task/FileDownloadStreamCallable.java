...
public class FileDownloadStreamCallable implements Callable<DownloadResult> {
    ...
    public FileDownloadStreamCallable(Map<String, Object> currentDownloadFileMap,
     ...
     *         // 4B -> 文件字节流长度
     */
    private DownloadResult loopSendFileDownloadStream(File currentDownloadFile, FileChannel fileChannel,
            SocketChannel socketChannel) throws IOException {
        int originStreamDataIndex = 4 + 1 + 4 + 4;
        // 包序号
        Integer packetIndex = 1;
        ByteBuffer byteBuffer = ByteBuffer
                .allocate(Integer.parseInt(NioServerContext.getValue(BasicConstant.DOWNLOAD_SEND_BYTE_BUFFER)));
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            // 封装文件发送流
            packetIndex = this.encapsulationStreamData(sumReadFileSize, currentDownloadFile.length(), packetIndex,
                    byteBuffer, sendBytes);
            // 再次包装文件流
            ...
            if (!result) {
                ...
                log.info(
                        "[ " + LocalTime.formatDate(LocalDateTime.now())
                                + " ] FileDownloadStreamCallable | --> 关闭文件下载通道(FileChannel)成功, file = {}, thread = {}",
                        currentDownloadFile.getName(), Thread.currentThread().getName());

                // 如果不是关闭状态，则数据发送失败尤其它原因导致，则关闭socketChannel
                if (!socketChannel.socket().isClosed()) {
                    NioServerContext.closedAndRelease(socketChannel);
                    return DownloadResult.generate("文件 [" + currentDownloadFile + "] 下载线程 ["
                            + Thread.currentThread().getName() + "] 发送数据第 [" + packetIndex + "] 个包数据失败", Boolean.FALSE);
                } else {
                    // 如果socketChannel状态为关闭，则为socketChannel所在线程被中断引起，此处不做任何操作，直接关闭文件读取通道
                    return DownloadResult.generate("文件 [" + currentDownloadFile + "] 下载线程 ["
                            + Thread.currentThread().getName() + "] 发送数据第 [" + packetIndex + "] 个包时由于下载线程被中断,已停止数据发送",
                            Boolean.FALSE);
                }
            }
             ...
             * " ] FileDownloadTransportStreamHandler | --> send file = [{}], packetIndex = [{}] download stream data success, thread = {}"
             * ,
             * currentDownloadFile.getName(), packetIndex,
     ...
     */
    private Integer encapsulationStreamData(int sumReadFileSize, long fileSize, Integer packetIndex,
            ByteBuffer byteBuffer, byte[] sendBytes/* , byte[] sendMessageBytes, byte[] sendMessageLengthBytes */) {
